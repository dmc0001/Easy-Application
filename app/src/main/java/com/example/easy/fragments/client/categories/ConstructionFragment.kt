...
@AndroidEntryPoint
class ConstructionFragment : BaseCategoryFragment() {
    @Inject
    lateinit var firestore: FirebaseFirestore

    private val constructionViewModel by viewModels<BaseCategoryViewModel> {
        BaseCategoryProviderFactory(firestore, Category.Construction)
    }
    ...
    override fun onViewCreated(view: View, savedInstanceState: Bundle?) {
        ...
        viewLifecycleOwner.lifecycleScope.launch {
            repeatOnLifecycle(Lifecycle.State.STARTED) {
                constructionViewModel.specialJobs.collectLatest { resource ->
                    when (resource) {
                        is Resource.Loading -> {
                            showLoading()
                        }

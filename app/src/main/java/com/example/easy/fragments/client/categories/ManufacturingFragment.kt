...
class ManufacturingFragment:BaseCategoryFragment() {
    ...
    private val manufacturingViewModel by viewModels<BaseCategoryViewModel> {
        BaseCategoryProviderFactory(firestore, Category.Manufacturing)
    }
    ...
    override fun onViewCreated(view: View, savedInstanceState: Bundle?) {
        ...
        viewLifecycleOwner.lifecycleScope.launch {
            repeatOnLifecycle(Lifecycle.State.STARTED) {
                manufacturingViewModel.specialJobs.collectLatest { resource ->
                    when (resource) {
                        is Resource.Loading -> {
                            showLoading()
                        }

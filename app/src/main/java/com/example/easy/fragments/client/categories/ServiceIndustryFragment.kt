...
@AndroidEntryPoint
class ServiceIndustryFragment : BaseCategoryFragment() {
    @Inject
    lateinit var firestore: FirebaseFirestore

    private val serviceIndustryViewModel by viewModels<BaseCategoryViewModel> {
        BaseCategoryProviderFactory(firestore, Category.ServiceIndustry)
    }
    ...
    override fun onViewCreated(view: View, savedInstanceState: Bundle?) {
        ...
        viewLifecycleOwner.lifecycleScope.launch {
            repeatOnLifecycle(Lifecycle.State.STARTED) {
                serviceIndustryViewModel.specialJobs.collectLatest { resource ->
                    when (resource) {
                        is Resource.Loading -> {
                            showLoading()
                        }
